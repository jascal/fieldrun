//! Tier B — composition, Gemma 3. The Gemma-2 stack (√d embedding scale, the four-norm sandwich per layer, GeGLU,
//! grouped-query attention with head_dim ≠ d/H, RMSNorm as x·(1+w)) with four precise changes that define Gemma 3:
//!   1. QK-norm — a per-head RMSNorm (over head_dim, (1+w) baked) applied to the query and key projections *before*
//!      RoPE; it replaces Gemma 2's attention-logit soft-cap.
//!   2. No soft-capping — neither the attention logits nor the final logits are capped.
//!   3. A 5:1 sliding:full layer pattern (Gemma 2 alternated 1:1); the per-layer sliding flag is packed into `config`.
//!   4. Dual-base RoPE — sliding (local) layers use θ≈10k, full (global) layers use θ≈1M, over the same head_dim.
//! A faithful Rust port of `Gemma3ForCausalLM`; validated top-1 against the torch reference on a tiny random-init
//! instance (the faithfulness gate — no gated download needed). Weights stay f16/f32 in RAM and upcast per matmul.
...
pub struct Gemma3 {
    ...
    inv_global: Vec<f32>,
    window: usize,
    sliding: Vec<bool>, // per-layer: true = sliding-window (local) attention, false = full (global)
    tied: bool,         // unembed = tied embedding when true, else a separate lm_head
...
impl Gemma3 {
    pub fn new(b: Bundle, route: f32, kv_int8: bool) -> Gemma3 {
        // config: [n_layer, H, nkv, hd, d, ffn, vocab, tied, sliding_window, <n_layer sliding flags>]
        ...
        let tied = b.config[7] != 0;
        let window = b.config[8] as usize;
        let sliding: Vec<bool> = (0..n_layer).map(|l| b.config[9 + l] != 0).collect();
        // config_f: [theta_local, theta_global, eps, qscalar, escale(√d)]
        let (theta_local, theta_global, eps, qscalar, escale) = (b.config_f[0] as f32, b.config_f[1] as f32,
            b.config_f[2] as f32, b.config_f[3] as f32, b.config_f[4] as f32);
        let inv = |theta: f32| (0..hd / 2).map(|j| 1.0 / theta.powf(2.0 * j as f32 / hd as f32)).collect();
        Gemma3 { b, n_layer, h, nkv, hd, d, eps, scale: qscalar.powf(-0.5), escale,
                 inv_local: inv(theta_local), inv_global: inv(theta_global), window, sliding, tied, route, kv_int8 }
    }
    ...
    /// QK-norm: RMS-normalise each head's head_dim slice (over hd) and scale by the (1+w)-baked per-dim weight. Applied
    /// to the q/k projections (n_heads heads of width hd packed per row) before RoPE.
    fn qk_norm(&self, x: &mut Array2<f32>, name: &str, n_heads: usize) {
        let w = self.b.arr1o(name); // length hd, (1+w) baked
        let hd = self.hd;
    ...
    fn rope(&self, x: &mut Array2<f32>, n_heads: usize, pos0: usize, inv: &[f32]) {
    ...
    fn hidden(&self, ids: &[i64]) -> Array2<f32> {
        ...
        let rep = h / nkv;
        let emb = self.b.rows_f32("embed", ids); // upcast only the looked-up rows
        ...
        for l in 0..self.n_layer {
            ...
            self.qk_norm(&mut q, &format!("{p}self_attn.q_norm"), h);   // QK-norm before RoPE
            self.qk_norm(&mut k, &format!("{p}self_attn.k_norm"), nkv);
            let inv = self.inv_for(l);
            self.rope(&mut q, h, 0, inv);
            self.rope(&mut k, nkv, 0, inv);
            ...
            for head in 0..h {
                ...
                let mut scores = qh.dot(&kh.t()) * self.scale; // no soft-cap in Gemma 3
                for i in 0..seq {
                    for j in 0..seq {
    ...
    fn explanation(&self, ids: &[i64]) -> crate::explain::Explanation {
        ...
        for l in 0..self.n_layer {
            ...
            let mut k = self.b.mm(&a, &format!("{p}self_attn.k_proj"));
            let v = self.b.mm(&a, &format!("{p}self_attn.v_proj"));
            self.qk_norm(&mut q, &format!("{p}self_attn.q_norm"), h);
            self.qk_norm(&mut k, &format!("{p}self_attn.k_norm"), nkv);
            let inv = self.inv_for(l);
            self.rope(&mut q, h, 0, inv);
            self.rope(&mut k, nkv, 0, inv);
    ...
    /// nkv*hd) and attending over the cache with Gemma 3's QK-norm + per-layer sliding window. cur = first-row position.
    fn forward_block(&self, emb: &Array2<f32>, cur: usize, kc: &mut [Array2<f32>], vc: &mut [Array2<f32>]) -> Array2<f32> {
        ...
        for l in 0..self.n_layer {
            ...
            self.qk_norm(&mut k, &format!("{p}self_attn.k_norm"), nkv);
            let inv = self.inv_for(l);
            self.rope(&mut q, h, cur, inv);
            self.rope(&mut k, nkv, cur, inv);
            ...
            for head in 0..h {
                ...
                for i in 0..m {
                    let abs = cur + i;
                    for j in 0..klen {
                        if j > abs || (sliding && j + self.window <= abs) {
                            scores[[i, j]] = -1e30;
                        }
                    }
                }
    ...
    /// `forward_block` with an int8 KV cache (GQA width, per-kv-head scale).
    ...
    fn head_argmax(&self, xfn: &Array2<f32>) -> i64 {
        ...
        let logits = self.b.rowdot_f32(self.unembed(), &last); // no final soft-cap; argmax unaffected anyway
