...
| `worst_case2.py` | **which functional controls the tax** (Grok's controlled-skew families): across 23 Dirichlet/mixture/power-law distributions, median recoverable rank tracks **min(exp(H),d) at Spearman +0.94** vs +0.86 for 95%-support — `exp(H_output)` wins. The mixture head/tail split (head ρ/d 0.08 regardless of tail ρ/d 1.00) unifies per-token `info_rank` with the aggregate law. Plots `worst_case2.png`. |
