...
OUT_DIR = "/tmp/g3tiny"
IDS_PATH = "/tmp/g3_holdout.json"
REF_PATH = "/tmp/g3_torch_preds.json"
CTX = 16
N_EVAL = 60
SEQ_LEN = 96
...
def build():
    ...
    cfg = Gemma3TextConfig(
        ...
        num_hidden_layers=7,      # layer 5 -> full ((5+1)%6==0); last forced full; 0-4 sliding -> both paths exercised
...
def compare(dump_path):
    ...
    print(f"[gemma3_ref] fieldrun vs torch Gemma3ForCausalLM: {agree}/{n} top-1 agree ({100*agree/n:.1f}%)")
    if agree != n:
